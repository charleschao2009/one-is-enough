...
public class FrontPageActivity extends FragmentActivity implements OnPageChangeListener{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		TypeModel model;
		for(int i = 0;i<10;i++){
			model  = new TypeModel("00"+i,"charles"+i);
			list.add(model);
		}
	...
	private void InitFragment(List<TypeModel> lists) {
		...
		for (int pages = 0; pages < lists.size(); pages++) {
			FrontPageFragment fragment = new FrontPageFragment(lists.get(pages));
